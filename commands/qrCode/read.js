...
module.exports = async function handleQrRead() {
	...
	try {
		...
		if (value.length < 1) {
			await sock.sendMessage(
				sender,
				{
					text: botLabel(`Tidak ada isi di qr code tersebut`)
				},
				{ quoted: msg }
			);
		}
		...
		await sock.sendMessage(
			...
			{
				text: botLabel(
					`Tidak bisa membaca qr code. Pastikan gambar mengandung qr code, dan jelas. Atau coba lapor admin bot`
				)
			},
			{ quoted: msg }
		);
