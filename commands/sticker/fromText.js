...
module.exports = async function fromTextToSticker() {
	...
	if (!match || match[1].length < 1) {
		await sock.sendMessage(
			...
			{
				text: botLabel(
					"Untuk membuat teks menjadi stiker gunakan format •teksnya•. Contoh\n*ubah teks •teksnya• jadi stiker*"
				)
			},
			{ quoted: msg }
		);
	...
	if (isEmoji.test(match[1].trim())) {
		await sock.sendMessage(
			sender,
			{
				text: botLabel("Emoji tidak bisa dijadikan stiker")
			},
			{ quoted: msg }
		);
		return;
